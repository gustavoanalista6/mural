<div id="content-curso" class="hidden">
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gray-100 text-gray-700 uppercase text-xs tracking-wider">
                        ...
                        <th class="py-4 px-6 text-left">Curso</th>
...
<script>
    const btnNovoCurso = document.getElementById('btn-create');

    btnNovoCurso.addEventListener('click', openCreateCursoModal);

    function openCreateCursoModal() {
        // Evita abrir dois modais
        if (document.getElementById('modal-create-curso')) return;

        const modal = document.createElement('div');
        modal.id = 'modal-create-curso';
        modal.className =
        ...
        modal.innerHTML = `
            <div class="bg-white w-full max-w-md rounded-xl shadow-lg p-6 relative animate-fade-in">
                ...
                <form id="form-create-curso" class="space-y-4">
        ...
        // Fechar modal
        modal.querySelector('.btn-close').onclick = () => closeCursoModal(modal);
        modal.querySelector('.btn-cancel').onclick = () => closeCursoModal(modal);

        // Fechar clicando fora
        modal.addEventListener('click', e => {
            if (e.target === modal) closeCursoModal(modal);
        });

        // Submit (exemplo)
        modal.querySelector('#form-create-curso').addEventListener('submit', e => {
            e.preventDefault();

            const formData = new FormData(e.target);

            console.log('Curso:', formData.get('curso'));
            console.log('Logo:', formData.get('logo'));
            console.log('Filial:', formData.get('filial'));

            // Aqui entra fetch / axios / Laravel
            closeCursoModal(modal);
        });
    }

    function closeCursoModal(modal) {
